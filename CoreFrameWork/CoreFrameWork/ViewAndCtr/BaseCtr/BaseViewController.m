...
- (void)viewWillAppear:(BOOL)animated {
	...
	if ([self showCustomTitleBarView]) {
        ...
        if ( ! self.isAppeared) {//这里默认是要显示navibar的!
            ...
            statusView.backgroundColor = RGB(22,177,131);
  ...
  //  [Login sharedInstance].currentViewController = self;
}
...
- (void)configBackButton {
    ...
    if (BackTypeDefault == self.backType) { //设置返回按钮(默认)
		...
		NSAssert(YES, @"self.backType = [%ld] 不支持该类型！", (long)self.backType);
	}
}
...
-(void)backSecondView
{
    UIViewController * ctr = [self backViewController];
    if (ctr.presentingViewController) {
            [ctr.presentingViewController dismissViewControllerAnimated:NO completion:nil];
    }
}
...
- (UIViewController *)presentViewController:(NSString *)className withParams:(NSDictionary *)paramDict {
    ...
    [navigationController.navigationBar setTintColor:[UIColor whiteColor]];//这个控制返回箭头按钮的颜色
